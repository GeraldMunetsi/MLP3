...
class SplineTemporalDecoderPhysics(nn.Module):
    ...
    def forward(self, z: torch.Tensor, rho_raw: torch.Tensor) -> tuple:
        ...
        Args:
            ...
            rho_raw : (batch,)            — initial infected fraction in [0,1], I0 = rho*N
        ...
        all_rates       = torch.cat([ones, retention_rates], dim=1)  # include r0=1
        cum_product     = torch.cumprod(all_rates, dim=1)             # guarantees monotone decreasing
        S_coeffs        = S_0 * cum_product
        S_pred          = self.spline_S(S_coeffs)                    # (batch, n_timepoints)

        # ── I: fraction of ever-infected currently infected
        # Predict B-spline coefficients
        delta_raw = self.predict_I_coeffs(z)                 # (batch, n_knots)
        delta     = -F.softplus(delta_raw)  
        # compute cumulative product in log-space to avoid in-place
        g_cumsum = torch.cumsum(delta, dim=1)
        g_all = torch.exp(g_cumsum)

        # pin first element to 1 without in-place
        g_coeffs = torch.cat([torch.ones_like(g_all[:, :1]), g_all[:, 1:]], dim=1)                           # pin g(0)=1
        g_spline  = self.spline_I(g_coeffs)                 # smooth curve (batch, n_timepoints)
        g         = g_spline.clamp(0.0, 1.0)               # enforce 0 <= g <= 1

        # ── Compute I and R using population conservation
        ever_infected = self.N - S_pred                     # always >= 0
